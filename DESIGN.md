...
- **screen** — 画面。部位は `text` / `text-muted` / `background` / `surface` / `border` / `accent`（リンク・強調テキスト）
- **button** — 操作。variant は役割そのもの（`primary` / `danger` / `neutral`）。色の部位は `surface` / `text` / `ring`。非色は `text-typography`（=`label`）・`rounded`（=`md`）・`padding-block` / `padding-inline`（=`sm` / `lg`）
...
- **input** — 入力欄。色の部位は `surface` / `border` / `text` / `placeholder` / `ring`。非色は `text-typography`（=`body-md`）・`rounded`（=`sm`）・`padding-block` / `padding-inline`（=`sm` / `md`）

値の参照先は、その値種別に役割が定義されているかで変わる。**色は役割層トークンを経由し、primitive を直接参照しない**。族の付け替えを `roles` の1行に閉じ、component に色相族を知らせないためである。**typography / rounded / spacing / shadow は尺度のキーを直接参照する**。これらの値種別にはまだ役割を定義しておらず、経由すべき役割層トークンが存在しない（Roles 節）。

theme（light/dark）や state（hover / focus 等）は、値と意味のどちらも持たない **modifier**。同じ意味単位の「見え方」を分岐させるだけで別物にはしないので、`-dark` のようにトークン名へ焼き付けず、component トークンの分岐キーとして持つ。theme は `light:` / `dark:`、state はそれに重ねて `<state>-light:` / `<state>-dark:`。theme・state で変わるのは色だけなので、分岐を持つのも色の行だけ。state の分岐は対話的 component にだけ足す。現在は button が `hover` / `active` / `focus` / `disabled`、input が `hover` / `focus` / `disabled` / `invalid`。note / badge / card / screen は非対話なので state を持たない。

focus は面や縁の張り替えではなく**リング**（`ring` 部位、outline 2px・offset 2px）で示す。ring の色は variant の意味と独立（「キーボードがここにいる」の表示）なので全 variant とも primary 族。ring × screen の地は非テキストの 3:1 を `check:contrast` が検証する。ブラウザ既定のフォーカスリングを `outline: none` で消してはならない。トークンの ring への**置き換え**だけを許す（既定リングの色は地とのコントラストを保証しないため、検証可能な色に差し替える目的に限る）。`disabled` の文字×地は WCAG の inactive 例外によりコントラスト要件（AA）の対象外で、`check:contrast` も除外する。ただし **disabled でも役割の色相族は保つ**（button-danger の disabled は danger の淡い段）。無彩へ落とすと variant 間の区別が消え、利用者が学習した役割⇔色相の対応（意味の発生源）を state が破壊してしまうため。
...
- `outline: none` でフォーカスリングを消さない。許されるのはトークンの `ring` への置き換えだけ
...
- component トークンの値は、色なら役割層トークン（`<役割>-color-<段>`）を参照し、primitive を直接参照しない。非色（typography / rounded / spacing / shadow）なら尺度のキー（`md` / `body-sm` 等）を直接参照する。役割を定義していない値種別には、経由すべき役割層トークンが無い
