...
- **page** — 画面の地・構造。部位は `text` / `text-muted` / `background` / `surface` / `border` / `accent`（リンク・強調テキスト）
...
- **card** — 面の分離。部位は `surface` / `border`。非色は `rounded`（=`md`）・`padding`（=`lg`）・`shadow`（=`sm`。Elevation の「面の分離」に対応）。中身の文字は page の text を使う
